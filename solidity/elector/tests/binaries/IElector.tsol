...
interface IElector {
    ...
    function recover_stake_gracefully(uint64 query_id, uint32 elect_id, int32 chainId) external functionID(0x47657425) internalMsg;

    /// @dev Allow getting elector info
    function get_elect_at(uint64 query_id, int32 chainId) external functionID(0x47657426) internalMsg;

    /// @dev Confirmation from configuration smart contract
    function config_set_confirmed_ok(uint64 query_id, int32 chainId) external functionID(0xee764f4b) internalMsg;
    function config_set_confirmed_err(uint64 query_id, int32 chainId) external functionID(0xee764f6f) internalMsg;
    ...
    function receiveBonuses(int32 chainId) external internalMsg;
}
