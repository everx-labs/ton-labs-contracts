...
echo SET MULTIBALLOT DEBOT ADDRESS
mb_deobt=$(cat ../multiballot/address.log)
$CLI_PATH/tonos-cli call $debot_address setMultiballotDebot "{\"adr\":\"$mb_deobt\"}" --sign $filenamekeys --abi $filenameabi
echo SET PROPOSALROOT DEBOT ADDRESS
pr_deobt=$(cat ../proposalRoot/address.log)
$CLI_PATH/tonos-cli call $debot_address setProposalDebot "{\"adr\":\"$pr_deobt\"}" --sign $filenamekeys --abi $filenameabi
