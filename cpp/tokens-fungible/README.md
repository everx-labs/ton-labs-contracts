...
## 4. Build contracts
### 4.1 Generate .abi.json file for each contract
```
clang++ -target tvm -export-json-abi --sysroot $TVM_INCLUDE_PATH RootTokenContract.cpp -o RootTokenContract.abi.json
clang++ -target tvm -export-json-abi --sysroot $TVM_INCLUDE_PATH TONTokenWallet.cpp -o TONTokenWallet.abi.json
```
### 4.2 Build and link contracts
```
tvm-build++.py --abi RootTokenContract.abi RootTokenContract.cpp -o RootTokenContract.tvc
tvm-build++.py --abi TONTokenWallet.abi TONTokenWallet.cpp -o TONTokenWallet.tvc
```
